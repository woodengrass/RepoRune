...
確認設計的實作**（`mark_possibly_stale`，見 `rune.core.semantic.worker`）。**第十一輪：使用者要求
`model` 空字串也一律算設定錯誤**——專案部署完成後理當已正確填入設定，不該因為空字串剛好是
`SemanticConfig` 的預設值就特殊放行；provider 健康檢查因此把「`semantic.enabled=true` 但 `model`
